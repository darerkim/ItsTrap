...
public class JetPackController : FollowPlayer
{
    ...
    void Update()
    {
        ...
        else if (Input.GetAxisRaw("Horizontal") < 0)
        {
            ...
            transform.rotation = Quaternion.Lerp(transform.rotation, Quaternion.Euler(-110, 0, 0), spinSpeed);
        }
        else if (Input.GetAxisRaw("Horizontal") == 0)
        {
            transform.position = Vector3.Lerp(transform.position, tf_Player.position - new Vector3(currentPos.x, currentPos.y, 0f), speed);
            transform.rotation = Quaternion.Lerp(transform.rotation, Quaternion.Euler(-55, 0, 0), spinSpeed);
        }
    }
}
